...
import { syncInvoiceToSheet, debugSheetConnection } from "./googleSheets.js";
...
export async function registerRoutes(
  ...
  // TEMPORARY DEBUG ROUTE — remove after sheet sync is confirmed working
  ...
  app.post(api.invoices.create.path, async (req, res) => {
    try {
      ...
      await syncInvoiceToSheet(invoice, input.items as any); // must await — Vercel kills background work after response is sent
  ...
  app.patch("/api/invoices/:id", async (req, res) => {
    try {
      ...
      await syncInvoiceToSheet(updatedInvoice, finalItems as any); // must await — Vercel kills background work after response is sent
    ...
    try {
      ...
      if (status === "accepted") {
        try {
          ...
          await syncInvoiceToSheet(newInvoice, items as any); // must await — Vercel kills background work after response is sent
